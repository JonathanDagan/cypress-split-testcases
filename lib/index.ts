...
async function generateTestFiles(testPath:string) {
    ...
    .readFileSync(path.join(__dirname, testPath, TEMPLATE_FOLDER, TEMPLATE_FILENAME))
    .toString();
    const configTemplate = fs
    .readFileSync(path.join(__dirname, testPath, TEMPLATE_FOLDER, CONFIG_FILENAME))
    .toString()
    
    chunks.forEach((chunk, index) => {
        const folderPath = path.join(__dirname, testPath, TEST_CASE_FOLDERNAME + index);
        fs.ensureDirSync(folderPath);
        fs.writeFileSync(path.join(folderPath, TEMPLATE_FILENAME), specTemplate);
        fs.writeFileSync(
